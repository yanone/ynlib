...
class Color(object):
    ...
    def lighten(self, value):
        ...
        if self.type == "RGB":
            ...
            return Color(
                RGB=(
                    Interpolate(self.R, other, value),
                    Interpolate(self.G, other, value),
                    Interpolate(self.B, other, value),
                )
            )
        elif self.type == "CMYK":
            other = self.min
            return Color(
                CMYK=(
                    Interpolate(self.C, other, value),
                    Interpolate(self.M, other, value),
                    Interpolate(self.Y, other, value),
                    Interpolate(self.K, other, value),
                )
            )
    ...
    def calcHex(self):
		...
		"""
        self.hex = (
            str(hex(self.R)[2:]).zfill(2) + str(hex(self.G)[2:]).zfill(2) + str(hex(self.B)[2:]).zfill(2)
        ).upper()
    ...
    if p == 0:
        ...
        for i in range(len(colors)):
            ...
            if before < p < after:
                ...
                return (
                    int(colors[i][0:2], 16) / 255.0,
                    int(colors[i][2:4], 16) / 255.0,
                    int(colors[i][4:6], 16) / 255.0,
                )
            elif p == after:
                return (
                    int(colors[i + 1][0:2], 16) / 255.0,
                    int(colors[i + 1][2:4], 16) / 255.0,
                    int(colors[i + 1][4:6], 16) / 255.0,
                )
...
def RGBtoHex(color):
	...
	"""
    return (
        str(hex(int(color[0] * 255))[2:]).zfill(2)
        + str(hex(int(color[1] * 255))[2:]).zfill(2)
        + str(hex(int(color[2] * 255))[2:]).zfill(2)
    )
...
if __name__ == "__main__":
    color = Color(hex="6eff00")
    print(color.darkHalf())
